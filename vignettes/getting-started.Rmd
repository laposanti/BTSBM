...
vignette: >
  %\VignetteIndexEntry{Getting started with BTSBM: the R package for the BT-SBM paper}
...
This vignette shows a minimal workflow with the `BTSBM` functions. We use as illustrative dataset the 2017 Men's ATP season for convenience. 
...
The relabelled posterior draws can also be collapsed into a block-by-block table
of Bradley-Terry win probabilities. Weighting by the observed match counts makes
the summary emphasise the block pairs that were actually well observed in the data.
...
Ranking summaries are computed directly from the relabelled player-strength draws.
These intervals complement the lambda plot by showing posterior uncertainty on the
rank scale, where 1 is the strongest player.
